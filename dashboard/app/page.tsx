    ...
    <main id="top">
      <section className="hero">
        <div className="heroCopy">
          ...
          <p>PAA’s Data Analytics Team's  2026 rising juniors &amp; seniors, ordered by draft probability and projected round range with position context and model probabilities.</p>
        </div>
      ...
      <div className="accuracyNote"><BookOpenCheck size={20}/><div><p><strong>Before interpreting, read:</strong> Draft probability estimates the chance that a player is selected in any round. A 20% value means about one in five similar historical profiles were drafted. Players below their position threshold are projected undrafted. Round probabilities apply only if the player is drafted. Multiply draft probability by a round probability to estimate the overall chance for that range. Thresholds differ by position.</p><p><strong>Methodology:</strong> The project uses sophomore and junior production, usage, development, size, and school draft history. Position models estimate draft chance. A second model estimates round range for drafted players. Results were tested on a later-season holdout. The model does not know future injuries, role changes, transfers, declarations, testing results, or team needs.</p></div></div>
      ...
      <section id="board" className="section">
        ...
        <div className="boardLayout">
          ...
          {selected && <aside className="prospectCard" ref={seniorCardRef}>
            ...
            <div className="draftChance"><span>Draft probability</span><strong>{pct(selected.draftProbability, 1)}</strong><div className="track"><i style={{width:`${selected.draftProbability*100}%`}}/></div><small>Position threshold: {pct(selected.threshold)}</small></div>
